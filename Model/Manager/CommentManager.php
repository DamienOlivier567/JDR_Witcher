...
class CommentManager {
    ...
    public function getCommentsArticle(int $article_fk): array {
        $comments = [];
        $request = DB::getInstance()->prepare("SELECT * FROM witcher_jdr.comment WHERE article_fk = $article_fk ORDER BY id DESC");
    ...
    public function getCommentArticle(int $id): array {
        $comments = [];
        $request = DB::getInstance()->prepare("SELECT * FROM witcher_jdr.comment WHERE id = $id");
        $result = $request->execute();
        if($result) {
            $data = $request->fetchAll();
            foreach ($data as $comment_data) {
    ...
    public function add(Comment $comment): bool {
        $request = DB::getInstance()->prepare("
            INSERT INTO witcher_jdr.comment (title, content, date, user_fk, article_fk)
    ...
    public function update (Comment $comment): bool {
        $request = DB::getInstance()->prepare("UPDATE witcher_jdr.comment SET title = :title, content = :content, date = :date WHERE id = :id");
    ...
    public function delete (Comment $comment): bool {
        $id = $comment->getId();
        $request = DB::getInstance()->prepare("DELETE FROM witcher_jdr.comment WHERE id = $id");
        return $request->execute();
    }
}