...
class StoryManager {
    ...
    public function getAll(): array {
        $articles = [];
        $request = DB::getInstance()->prepare("SELECT * FROM witcher_jdr.article");
        ...
        if($result) {
            ...
            foreach ($data as $article_data) {
                ...
                if($user->getId()) {
                    $articles[] = new Story($article_data['id'], $article_data['title'],  $article_data['content'], $article_data['picture'], $user);
                }
            }
        }
        return $articles;
    ...
    public function getArticle($id) {
        $request = DB::getInstance()->prepare("SELECT * FROM witcher_jdr.article WHERE id = :id");
     ...
     * @return bool
     */
    public function add(Story $article): bool {
        $request = DB::getInstance()->prepare("
            INSERT INTO witcher_jdr.article (title, content, picture, user_fk)
     ...
     * @return bool
     */
    public function update (Story $article): bool {
        $request = DB::getInstance()->prepare("UPDATE witcher_jdr.article SET title = :title, content = :content, picture = :picture WHERE id = :id");
    ...
    public function delete (Story $article) {
        $id = $article->getId();
        $request = DB::getInstance()->prepare("DELETE FROM witcher_jdr.article WHERE id = $id");
        return $request->execute();
    }
}