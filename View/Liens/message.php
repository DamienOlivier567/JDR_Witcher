...
function getMessages(){
    ...
    $resultats = $request->query("SELECT message.date, user.pseudo, message.message FROM witcherjdr.message, 
    witcherjdr.user WHERE message.user_fk = user.id ORDER BY date DESC LIMIT 20");
...
function postMessage(){
    ...
    $db = new DB();
    $request = $db->getDbLink();
    $query = $request->prepare("INSERT INTO witcherjdr.message SET message = :message, user_fk = :user_fk");
    $query->execute([
        ":message" => $message,
        ":user_fk" => $user_fk
    ]);
